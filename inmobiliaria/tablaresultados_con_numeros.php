...
$query_resultados = "SELECT tipo, dormitorios, m2_construidos, precio, provincia, localidad, zona_o_barrio FROM inmuebles";
...
<table border="0" width="61%" align="center">
  <tr>
    <td width="41%"><?php echo "$totalRows_resultados inmuebles encontrados"; ?></td>
	<td width="59%"><?php $startRow_resultados++;?>
	<?php 
	if($totalRows_resultados!=0){
	if($totalRows_resultados-$startRow_resultados < $maxRows_resultados) {
	$intervalo2=($totalRows_resultados+1)-$startRow_resultados;}
	else {$intervalo2=$startRow_resultados+$maxRows_resultados-1;}
	if($intervalo2!=1)
		{echo $startRow_resultados . " al " . $intervalo2;}
	}?> </td>
  </tr>
</table>
<p>&nbsp;</p>
